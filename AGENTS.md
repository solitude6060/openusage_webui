...
- This repo is a GitHub fork of `robinebers/openusage`, so GitHub points `gh` at the parent repo by default and `gh pr create` fails with a "does not have the correct permissions" error. The default repo is pinned to this fork via `gh repo set-default solitude6060/openusage_webui`; if that resets, target it explicitly with `gh pr create -R solitude6060/openusage_webui ...`. There is intentionally no `upstream` remote — add one only to pull the original's updates, then remove it again. Note (verified 2026-06-28): upstream's `main` is now a from-scratch **Swift** rewrite with a disjoint history (no common ancestor — do not try to merge it). Our shared Tauri lineage lives on `upstream/tauri-legacy`, which is frozen and being retired; diff against that branch, not `main`, if you ever pull upstream changes.
